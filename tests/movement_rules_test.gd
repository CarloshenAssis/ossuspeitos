...
func _initialize() -> void:
	...
	for step in 120:
		...
		if velocity.length() > MovementRules.MAX_SPEED + 0.001:
			...
			return
	state["position"] = Vector3(MovementRules.ARENA_HALF_EXTENT, MovementRules.PLAYER_HEIGHT, 0.0)
	...
	if bounded_position.x > MovementRules.ARENA_HALF_EXTENT or bounded_velocity.x != 0.0:
